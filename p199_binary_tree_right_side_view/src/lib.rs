...
impl Solution {
    pub fn right_side_view_dfs(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        ...
        fn dfs(tree: Option<Rc<RefCell<TreeNode>>>, vec: &mut Vec<i32>, level: i32) {
            if let Some(node) = tree {
                let node = node.borrow();
                if level == vec.len() as i32 {
                    vec.push(node.val)
                }
                for i in (0..=1).rev() {
                    if let Some(leaf) = node[i].as_ref() {
                        dfs(Some(Rc::clone(leaf)), vec, level + 1);
                    }
                }
            }
        }
    ...
    pub fn right_side_view_bfs(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        ...
        while !q.is_empty() {
            let num_level_els = q.len();
            result.push(q[q.len() - 1].borrow().val);
            for _ in 0..num_level_els {
...
mod tests {
    ...
    #[test]
    fn case_01() {
        let vec = vec![Some(1), Some(2), Some(3), None, Some(5), None, Some(4)];
        let root = from_vec_to_bt(&vec);
        let expected = vec![1, 3, 4];
        let result1 = Solution::right_side_view_dfs(root.clone());
        let result2 = Solution::right_side_view_bfs(root.clone());
